    ...
    for (int r = -widNum / 2.0; r < widNum / 2.0; r++)
      for (int s = -widNum / 2.0; s < widNum / 2.0; s++) {
        ...
        for (int t = -10; t < heiNum; t++) {
          ...
          if ((Eigen::Vector2d(temp_x, temp_y) - Eigen::Vector2d(x, y))
                  .norm() <= radius) {
            pt_random.x = temp_x;
            pt_random.y = temp_y;
            pt_random.z = temp_z;
...
void RandomInclinedColumn() {
  ...
  for (int i = 0; i < _obs_num && ros::ok(); i++) {
    ...
    int widNum = ceil((w * inf) / _resolution);
    // double radius = (w * inf) / 2;
    Eigen::AngleAxisd R_v(rand_theta_tilt,
                          Eigen::Vector3d(cos(rand_theta), sin(rand_theta), 0));
    ...
    for (int r = -widNum / 2.0; r < widNum / 2.0; r++)
      for (int s = -widNum / 2.0; s < widNum / 2.0; s++) {
        int heiNum = ceil(h / _resolution);
        for (int t = -10; t < heiNum; t++) {
          Eigen::Vector3d temp_p(x + (r + 0.5) * _resolution + 1e-2,
                                 y + (s + 0.5) * _resolution + 1e-2,
                                 (t + 0.5) * _resolution + 1e-2);
          temp_p = R_m * (temp_p - Eigen::Vector3d(x, y, 0)) +
...
void clickCallback(const geometry_msgs::PoseStamped &msg) {
  ...
  localMap_pcd.header.frame_id = _frame_id_;
...
void pubPoints() {
  ...
  globalMap_pcd.header.frame_id = _frame_id_;
...
int main(int argc, char **argv) {
  ...
  n.param("frame_id", _frame_id_, std::string("world"));
